...
class Inference:
    def __init__(
        ...
        """
        This class provide method to predict feature events
        :param N_steps: int, number of timestamp to use
        :param model_weights_path: Str, default to None,
            which means model will be loaded  from data/.
            Otherwise, it will load from the specified path.
        :param detail_output_path: Str, default to None,
            which means inference detail will be saved to data/inference.csv
            Otherwise, it will save to the specified path.
        :param halfhour_output_path: Str, default to None,
            which means inference half-hour data will be saved to
        ...
        self.halfhour_output_path = self._get_halfhour_output_path(halfhour_output_path)
    ...
    def load_model(self):
        tsmodel = TSModel(
            N_steps=self.N_steps, N_features=DataLoader.NUM_FEATURES, lr=0.001
        )
    ...
    def inference(self):
        """
        This method will predict future 7 days event occurrence timestamps and
        associated x,y coordinates
        :return: dataframe with 3 columns, event_time, x, y
        """
        ...
        while latest_time < end:
            ...
            nn_input_data = self.data_loader.gen_x_y_by_index(
                len(self.data_loader.data_array), return_label=False
            )[0]
            ...
            outputs.append({"event_time": latest_time, "x": x_scaled, "y": y_scaled})

            array = self.data_loader.construct_input_data(
                time=latest_time, x=x, y=y, time_delta=time_delta
            )
    ...
    def generate_halfhour_result(self):
        """
        This method will predict future 7 days event occurrence for each
        30-minutes intervals
        :return: dataframe with 2 columns, time beginning, number of events
        """
        ...
        while cur < end_:
            ...
            data_ = {
                "time_beginning": cur.strftime("%Y-%m-%d %H:%M:%S"),
                "number_of_event": res,
            }
            data.append(data_)
            logger.info(data_)
    ...
    def query_result(self, from_datetime: str, to_datetime: str):
        """
        This method can query inference data, and return the data that falls
        into the inverval
